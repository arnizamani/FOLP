...
type TRuleFunc = Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
...
data TRule = FormInspect | TInspect | FInspect | AReduce | EReduce | Rewrite | PiInspectP | SigmaInspectP | DeltaInspect |
             ...
             AAndComp | AOrComp | AArrowComp | AReduceP | EReduceP | Substitution | NoRule
             deriving (Eq)
...
instance Show TRule where
    show Substitution  = "Substitution"
...
recall :: [TRule] -> Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
recall usedrules c@(Ctx (_, (ts, nts, cs, ncs, _, _), ps, _, _, sargs, (_, costs), _, _)) e g d =
    do (matches, rule) <- case ps of
                              T -> if not (elem TRecall usedrules)
                                       then return ([], TRecall)
                                       else return ([getPatternMapping e p [] | p <- ts], TRecall)
                              NT -> if not (elem CRecall usedrules)
                                        then return ([], CRecall)
                                        else return ([getPatternMapping e p [] | p <- nts], CRecall)
       if not (null [m | Ok m <- matches]) 
           then do let state = (e, ENull, e)
                   return $ (Passed, makeProof state [FoundSol] rule)
           else return (Passed, [])


strengthen :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
strengthen c@(Ctx ((_, smap, _), memsegs, ps, _, _, sargs, (_, costs), model, _)) e g d =
    do let (ts, nts, cs, ncs, rws, (strns, _)) = memsegs
       let rewrittenexps = justLift [case getPatternMapping e b [] of
                                         Ok m -> Just $ replace m a
                                         _    -> Nothing | (a, b) <- strns]
       res <- sequence [do let state = (EImplies re e, ENull, e)
                           let goal = insertAssignMaps re
                           (rt, solved) <- solve c goal (pathCost costs state g Strengthen sargs) d
                           return $ (rt, makeProof state solved Strengthen) | re <- rewrittenexps, not $ opposite smap model re ps]
       if null res
           then return (Passed, [])
           else if all (== OPrune) (map fst res)
                    then return (OPrune, [])
                    else return $ (Passed, concat (map snd res))


weaken :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
weaken c@(Ctx ((_, smap, _), memsegs, ps, _, _, sargs, (_, costs), model, _)) e g d =
    do let (ts, nts, cs, ncs, rws, (_, weaks)) = memsegs
       let rewrittenexps = justLift [case getPatternMapping e a [] of
                                         Ok m -> Just $ replace m b
                                         _    -> Nothing | (a, b) <- weaks]
       res <- sequence [do let state = (EImplies re e, ENull, e)
                           let goal = insertAssignMaps re
                           (rt, solved) <- solve c goal (pathCost costs state g Weaken sargs) d
                           return $ (rt, makeProof state solved Weaken) | re <- rewrittenexps, not $ opposite smap model re ps]
       if null res
           then return (Passed, [])
           else if all (== OPrune) (map fst res)
                    then return (OPrune, [])
...
makeRewriteProof :: [TRule] -> Ctx -> WM -> TProofSize -> TProofSize -> [TExpOccPair] -> [(Exp, [Exp])] -> IO TResult
makeRewriteProof usedrules c@(Ctx (_, _, ps, _, _, sargs, (_, costs), _, _)) e g d ansub erwrtlkup = 
    do let rm   = [((se, idx), lookup se erwrtlkup) | (se, idx) <- ansub]
       let rm'  = [(se, m) | (se, Just m) <- rm]
       let rm'' = concat [[(se, m') | m' <- m] | (se, m) <- rm']
       let fappl = (\ret (se, substexp) ->
                        do (rt, proofs) <- ret
                           if not (rt == OPrune)
                               then do let substedexp = subst substexp [se] e
                                       let goal = insertAssignMaps substedexp
                                       let state = (EEquiv (fst se) substexp, ENull, e)
                                       {- putStrLn "At rewrite"
                                       putStrLn $ "g is: " ++ show g
                                       putStrLn $ "Rewriting to " ++ printExtendedExp (snd goal) (fst goal)
                                       putStrLn $ "Pathcost is " ++ (show (pathCost costs state g Rewrite sargs))
                                       putStrLn "Going to solve now" -}
                                       (rt2, solved) <- solve c goal (pathCost costs state g Rewrite sargs) d
                                       -- putStrLn $ "Returntype is " ++ show rt
                                       -- putStrLn "I got the following proofs:"
                                       -- mapM putStrLn $ map printProof solved
                                       return $ (rt2, (makeProof state solved Rewrite) ++ proofs)
                               else return (OPrune, []))
       foldl fappl (return (Passed, [])) rm''
...
rewrites :: [TRule] -> Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
rewrites usedrules c@(Ctx ((_, smp, _), (ts, nts, cs, ncs, rws, _), _, (sml, wml), _, _, _, _, _)) e g d = 
    do let ansubs' = subformulas e False
       let ansubs = [sub | sub <- ansubs', not $ track sub e] -- || (isDelta smp (fst sub) && expSize (expandedExp smp (fst sub)) <= sml) ] 
       let allsubs = nub $ map fst ansubs                                
       if elem Rewrite usedrules
           then do let rewritelkup = [(e', nub (exprewrite e' rws)) | e' <- allsubs]
                   makeRewriteProof usedrules c e g d ansubs rewritelkup
           else return (Passed, [])

substutiteA :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
substutiteA (Ctx (s@(svs, smp, (discovered, aux, occmp)), memsegs, ps, (sml, wml), pws, sargs, pm@(_, costs), model, coffs)) e g d =
    do let fappl1 =
               (\ret (sensvar, idx) ->
                    do (rt, proofs) <- ret
                       if not (rt == OPrune)
                           then do let expsens = case lookup sensvar smp of
                                                     Just exp -> exp 
                                                     _        -> error "error in inspect"
                                   let substedexps = [substForSensVars s es expsens | es <- getComprCombs expsens] ++ [(expsens, s)]
                                   let iteratewms = [let substsubexp = subst e' [(ESensVar (Ident sensvar) 1, idx)] e
                                                         goal = insertAssignMaps substsubexp
                                                         newAuxTree = substAuxTree (createAuxTree e') (sensvar, 1) aux
                                   ...
                                   let prunewms = [(goals, nsmp) | (goals, ((_, nsmp, _), _)) <- iteratewms]
                                   let fappl2 = 
                                           (\ret2 (preswm, (ns, sense')) ->
                                                do (rt2, proofs2) <- ret2
                                                   if not (rt2 == OPrune)
                                                       then do let (_, nsmap, _) = ns
                                                               let state = (ENull, EEqual (ESensVar (Ident sensvar) 1) sense', e)
                                                               (rt3, solved) <- solve (Ctx (ns, memsegs, ps, (sml, wml), pws ++ 
                                                                                      (delete (preswm, nsmap) prunewms), sargs, pm, model, coffs)) 
                                                                                      preswm 
                                                                                      (pathCost costs state g Substitution sargs) d
                                                               return (rt3, (makeProof state solved Substitution) ++ proofs2)
                                                       else return (OPrune, []))
                                   (rt4, proofs3) <- foldl fappl2 (return (Passed, [])) iteratewms
                                   return (rt4, proofs ++ proofs3)
                           else return (OPrune, []))
       foldl fappl1 (return (Passed, [])) (indexedList $ [v | v <- getVars' True e, case lookup v smp of
                                                                                        Just sexp -> (not $ isDelta smp sexp) || expSize sexp > sml
                                                                                        _ -> error "error in formula inspect"])

formulaInspect :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
                                                ...
                                                do (rt2, proofs2) <- ret2
                                                   if not (rt2 == OPrune)
                                                       then do let (_, nsmap, _) = ns
                                                               ...
                                                               return (rt3, (makeProof state solved FormInspect) ++ proofs2)
                                                       else return (OPrune, []))
                                   (rt4, proofs3) <- foldl fappl2 (return (Passed, [])) iteratewms
                                   return (rt4, proofs ++ proofs3)
                           else return (OPrune, []))
...
forallReduce :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
forallReduce c@(Ctx (_, _, ps, _, _, sargs, (_, costs), model, _)) e g d =
    do let sl = concat [[(ts, (EAnd (EForAll (Var v) (OptDom (dom \\ [i])) x) (ECtx x [Ctxt (Var v) i]))) | i <- dom] 
                                      | ts@((EForAll (Var v) (OptDom dom) x), idx) <- subformulas e False, not $ null dom || track ts e]
       let substedlst = [(subst se [ts] e) | (ts, se) <- sl]
       -- putStrLn (show (length substedlst))
       let fappl = (\ret sexp ->
                        do (rt, proofs) <- ret
                           if not (rt == OPrune)
                               then do let goal = insertAssignMaps sexp
                                       let state = (ENull, ENull, e)
                                       -- putStrLn $ "reducing " ++ printExtendedExp mp e ++ " to " ++ printExtendedExp (snd goal) (fst goal)
                                       (rt2, solved) <- solve c goal (pathCost costs state g AReduce sargs) d
                                       return $ (rt2, (makeProof state solved AReduce) ++ proofs)
                               else return (OPrune, []))
       foldl fappl (return (Passed, [])) substedlst


forallReducePlus :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
forallReducePlus c@(Ctx ((sv:svs, smp, disc), msegs, ps, ms, pwm, sargs, (rules, costs), model, cutoff)) e g d =
    do let sl = concat [if length dom > 1
                            then [(ts, (svs, (sv, (EForAll (Var v) (OptDom (dom \\ [i])) x)):smp, disc), EAnd (ESensVar (Ident sv) 1) (ECtx x [Ctxt (Var v) i])) | i <- dom]
                                      ...
                                      | ts@((EForAll (Var v) (OptDom dom) x), idx) <- subformulas e False, not $ null dom || track ts e]
       let substedlst = [(subst se [ts] e, nsmp) | (ts, nsmp, se) <- sl]
       let fappl = (\ret (sexp, nsmp) ->
                        do (rt, proofs) <- ret
                           if not (rt == OPrune)
                               then do let goal = insertAssignMaps sexp
                                       let (sv':svs', smp', _) = nsmp
                                       let state = (if sv' == sv
                                                        then ENull
                                                        else EEquiv (ESensVar (Ident (fst (head smp'))) 1) (snd (head smp')), ENull, e)
                                       let newc = Ctx (nsmp, msegs, ps, ms, pwm, sargs, (rules, costs), model, cutoff)
                                       (rt2, solved) <- solve newc goal (pathCost costs state g AReduceP sargs) d
                                       return $ (rt2, (makeProof state solved AReduceP) ++ proofs)
                               else return (OPrune, []))
       foldl fappl (return (Passed, [])) substedlst


existsReduce :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
existsReduce c@(Ctx (_, _, ps, _, _, sargs, (_, costs), model, _)) e g d =
    do let sl = concat [[(ts, (EOr (EExists (Var v) (OptDom (dom \\ [i])) x) (ECtx x [Ctxt (Var v) i]))) | i <- dom] 
                        ...
                        do (rt, proofs) <- ret
                           if not (rt == OPrune)
                               then do let goal = insertAssignMaps sexp
                                       ...
                                       return $ (rt2, (makeProof state solved EReduce) ++ proofs)
                               else return (OPrune, []))
       foldl fappl (return (Passed, [])) substedlst
       

existsReducePlus :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
existsReducePlus c@(Ctx ((sv:svs, smp, disc), msegs, ps, ms, pwm, sargs, (rules, costs), model, cutoff)) e g d =
    do let sl = concat [if length domp > 1
                        ...
                        do (rt, proofs) <- ret
                           if not (rt == OPrune)
                               then do let goal = insertAssignMaps sexp
                                       ...
                                       return $ (rt2, (makeProof state solved EReduceP) ++ proofs)
                               else return (OPrune, []))
       foldl fappl (return (Passed, [])) substedlst
...
forallDistribute :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
forallDistribute c@(Ctx ((_, smap, _), _, ps, _, _, sargs, (_, costs), model, _)) e g d =
    do let qes = [ts | ts@((EForAll _ (OptDom dom) _), idx) <- subformulas e False, not $ null dom || track ts e]
       let dexps = concat [case qexp of
                               ...
                               _ -> [] | ts@(qexp, idx) <- qes]
       let substedlst' = [subst se [ts] e | (ts, se) <- dexps]
       let substedlst = [sub | sub <- substedlst', case sub of (EAnd a b) -> True; _ -> not $ opposite smap model sub ps]
       let fappl = (\ret sexp ->
                        do (rt, proofs) <- ret
                           let goal = insertAssignMaps sexp
                           let state = (ENull, ENull, e)
                           (rt2, solved) <- solve c goal (pathCost costs state g ADistribute sargs) d
                           return $ (Passed, (makeProof state solved ADistribute) ++ proofs))
       foldl fappl (return (Passed, [])) substedlst
    

existsDistribute :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
existsDistribute c@(Ctx ((_, smap, _), _, ps, _, _, sargs, (_, costs), model, _)) e g d =
    do let qes = [ts | ts@((EExists _ (OptDom dom) _), idx) <- subformulas e False, not $ null dom || track ts e]
       let dexps = concat [case qexp of
                               ...
                               _ -> [] | ts@(qexp, idx) <- qes]
       let substedlst' = [subst se [ts] e | (ts, se) <- dexps]
       let substedlst = [sub | sub <- substedlst', not $ opposite smap model sub ps]
       let fappl = (\ret sexp ->
                        do (rt, proofs) <- ret
                           let goal = insertAssignMaps sexp
                           let state = (ENull, ENull, e)
                           (rt2, solved) <- solve c goal (pathCost costs state g EDistribute sargs) d
                           return $ (Passed, (makeProof state solved EDistribute) ++ proofs))
       foldl fappl (return (Passed, [])) substedlst
...
modelInspect :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
modelInspect c@(Ctx (_, _, _, _, _, sargs, (_, costs), model, _)) e g d =
    do let substedexps = [case p of
                        ...
                        do (rt, proofs) <- ret
                           if not (rt == OPrune)
                               then do let goal = insertAssignMaps sexp
                                       ...
                                       return $ (rt2, (makeProof state solved MInspect) ++ proofs)
                               else return (OPrune, []))
       foldl fappl (return (Passed, [])) substedexps


truthInspect :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
truthInspect c@(Ctx ((_, smp, _), _, ps, _, _, sargs, (_, costs), model, _)) e g d =
    do let sl = [subst ETrue [(ts, i)] e | (ts, i) <- subformulas e True, isSigma smp ts &&
       ...
       let fappl = (\ret sexp ->
                        do (rt, proofs) <- ret
                           if not (rt == OPrune)
                               then do let goal = insertAssignMaps sexp
                                       let state = (ENull, ENull, e)
                                       (rt2, solved) <- solve c goal (pathCost costs state g TInspect sargs) d
                                       return $ (rt2, (makeProof state solved TInspect) ++ proofs)
                               else return (OPrune, []))
       foldl fappl (return (Passed, [])) sl


falsityInspect :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
falsityInspect c@(Ctx ((_, smp, _), _, ps, _, _, sargs, (_, costs), model, _)) e g d =
    do let sl = [subst EFalse [(ts, i)] e | (ts, i) <- subformulas e True, isGamma smp ts && 
       ...
       let fappl = (\ret sexp ->
                        do (rt, proofs) <- ret
                           if not (rt == OPrune)
                               then do let goal = insertAssignMaps sexp
                                       let state = (ENull, ENull, e)
                                       (rt2, solved) <- solve c goal (pathCost costs state g FInspect sargs) d
                                       return $ (rt2, (makeProof state solved FInspect) ++ proofs)
                               else return (OPrune, []))
       foldl fappl (return (Passed, [])) sl


piInspect' :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
piInspect' c@(Ctx ((_, smp, _), _, ps, (sml, wml), _, sargs, (_, costs), model, _)) e g d =
    do let sl = [(ts, (EForAll (Var v) (OptDom (tail dom)) x))
       ...
       let fappl = (\ret sexp ->
                        do (rt, proofs) <- ret
                           if not (rt == OPrune)
                               then do let goal = insertAssignMaps sexp
                                       let state = (ENull, ENull, e)
                                       (rt2, solved) <- solve c goal (pathCost costs state g PiInspectP sargs) d
                                       return $ (rt2, (makeProof state solved PiInspectP) ++ proofs)
                               else return (OPrune, []))
       foldl fappl (return (Passed, [])) substedlst


substitutePi :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
substitutePi c@(Ctx ((_, smp, _), _, ps, (sml, wml), _, sargs, (_, costs), model, _)) e g d =
    do let sl = [(ts, (EForAll (Var v) (OptDom (tail dom)) x))
                                    | ts@(tse@(EForAll (Var v) (OptDom dom) x), idx) <- subformulas e False, 
                                                                                            (not $ null dom || track ts e) && 
                                                                                            isDelta smp x && let expexp = expandedExp smp tse
       ...
       let fappl = (\ret sexp ->
                        do (rt, proofs) <- ret
                           if not (rt == OPrune)
                               then do let goal = insertAssignMaps sexp
                                       let state = (ENull, ENull, e)
                                       (rt2, solved) <- solve c goal (pathCost costs state g Substitution sargs) d
                                       return $ (rt2, (makeProof state solved Substitution) ++ proofs)
                               else return (OPrune, []))
       foldl fappl (return (Passed, [])) substedlst

substituteDelta :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
substituteDelta c@(Ctx ((_, smp, _), _, ps, (sml, wml), _, sargs, (_, costs), model, _)) e g d =
    do let sl = [(ts, case eval model (expandedExp smp tse) of Just tv -> [tv]; _ -> []) | 
...
deltaInspect :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
       ...
       let fappl = (\ret sexp ->
                        do (rt, proofs) <- ret
                           if not (rt == OPrune)
                               then do let goal = insertAssignMaps sexp
                                       let state = (ENull, ENull, e)
                                       (rt2, solved) <- solve c goal (pathCost costs state g DeltaInspect sargs) d
                                       return $ (rt2, (makeProof state solved DeltaInspect) ++ proofs)
                               else return (OPrune, []))
       foldl fappl (return (Passed, [])) substedlst


substituteSigma :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
substituteSigma c@(Ctx ((_, smp, _), _, ps, (sml, wml), _, sargs, (_, costs), model, _)) e g d =
    do let sl = [(ts, (EExists (Var v) (OptDom (tail dom)) x))
...
sigmaInspect' :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
                        ...
                        do (rt, proofs) <- ret
                           if not (rt == OPrune)
                               then do let goal = insertAssignMaps sexp
                                       ...
                                       return $ (rt2, (makeProof state solved SigmaInspectP) ++ proofs)
                               else return (OPrune, []))
       foldl fappl (return (Passed, [])) substedlst
                         ...
                         in if null domp
                             then []
                                  ...
                                  | ts@(EExists (Var v) (OptDom dom') (EAnd x (EPred pred)), idx) <- subformulas e False, 
                                                                                                          isColorPred pred && (not $ null dom')] 
       let substedlst = [(subst se [ts] e) | (ts, se) <- sl1 ++ sl2]
       let fappl = (\ret sexp ->
                        do (rt, proofs) <- ret
                           if not (rt == OPrune)
                               then do let goal = insertAssignMaps sexp
                                       ...
                                       return $ (rt2, (makeProof state solved EAndComp) ++ proofs)
                               else return (OPrune, []))
       foldl fappl (return (Passed, [])) substedlst


sigmaOrComprehension :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
                        ...
                        do (rt, proofs) <- ret
                           if not (rt == OPrune)
                               then do let goal = insertAssignMaps sexp
                                       ...
                                       return $ (rt2, (makeProof state solved EOrComp) ++ proofs)
                               else return (OPrune, []))
       foldl fappl (return (Passed, [])) substedlst


piAndComprehension :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
piAndComprehension c@(Ctx (_, _, ps, _, _, sargs, (_, costs), model, _)) e g d =
    do let sl1 = concat [let domp = getDom dom' model pred EFalse
                         in if not $ null domp
                             then []
                        ...
                        do (rt, proofs) <- ret
                           if not (rt == OPrune)
                               then do let goal = insertAssignMaps sexp
                                       ...
                                       return $ (rt2, (makeProof state solved AAndComp) ++ proofs)
                               else return (OPrune, []))
       foldl fappl (return (Passed, [])) substedlst


piOrComprehension :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
                        ...
                        do (rt, proofs) <- ret
                           if not (rt == OPrune)
                               then do let goal = insertAssignMaps sexp
                                       ...
                                       return $ (rt2, (makeProof state solved AOrComp) ++ proofs)
                               else return (OPrune, []))
       foldl fappl (return (Passed, [])) substedlst


piArrowComprehension :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
piArrowComprehension c@(Ctx (_, _, ps, _, _, sargs, (_, costs), model, _)) e g d =
    do let sl = concat [let domp = getDom dom' model pred ETrue
                        in if null domp
                            then []
                            else [(ts, EForAll (Var v) (OptDom domp) x)] 
                        ...
                        do (rt, proofs) <- ret
                           if not (rt == OPrune)
                               then do let goal = insertAssignMaps sexp
                                       ...
                                       return $ (rt2, (makeProof state solved AArrowComp) ++ proofs)
                               else return (OPrune, []))
       foldl fappl (return (Passed, [])) substedlst
...
ruleApplications :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
ruleApplications c@(Ctx (_, _, _, _, _, _, (funcs, _), _, _)) wm g d =
    do let fappl = (\ret func -> 
                         do (rt, proofs) <- ret
                            if (rt /= OPrune)
                                then do (rt2, proofs2) <- func c wm g d
                                        if (rt2 /= OPrune)
                                            then return (rt2, proofs ++ proofs2)
                                            else return (OPrune, [])
                             else return (OPrune, []))
       foldl fappl (return (Passed, [])) funcs
...
prunePossible :: Ctx -> WM -> IO TResultType
prunePossible c@(Ctx ((_, smap, _), memsegs, ps, (_, wmlim), _, _, _, model, _)) e =
    do if expSize e > wmlim
           then return WMO
           else if ps == T
                    then do let (ts, nts, cs, ncs, _, _) = memsegs
                            if not $ null [m | Ok m <- [getPatternMapping e p [] | p <- nts]]
                                then return OPrune
                                else return Passed
                                {- else do let fullexp = expandedExp smap e
                                        if eval model fullexp == Just EFalse
                                            then do -- putStrLn $ (printExp (fst fullexp)) ++ " evaluates to false"
                                                    return OPrune
                                            else return Passed -}
                    else do let (ts, nts, cs, ncs, _, _) = memsegs
                            if not $ null [m | Ok m <- [getPatternMapping e p [] | p <- ts]]
                                then return OPrune
                                else return Passed
                                {- else do let fullexp = expandedExp smap e
                                        if eval model fullexp == Just ETrue
                                            then do -- putStrLn $ "going in: " ++ (printExp (fst wm))
                                                    -- putStrLn $ "coming out: " ++ (printExp (fst fullexp))
                                                    return OPrune
                                            else return Passed -}


solve :: Ctx -> WM -> TProofSize -> TProofSize -> IO TResult
solve c@(Ctx (s@(_, smap, _), msegs, psystem, memlims, prevwm, sargs@(le, p, u), pm@(_, costs), model, coffs)) wm g d =
    do -- putStrLn "At solve now"
       -- if expSize wm == 3
       --    then appendFile "biskvi.txt" {- ((show wm) ++ "\n") -} ((printExp wm) ++ "\n")
       --    else return ()
       -- putStrLn $ "g is: " ++ show g
       -- putStrLn $ "d is: " ++ show d
       prunetype <- prunePossible c wm
       if prunetype == WMO || prunetype == OPrune
           then do -- putStrLn $ "I prune " ++ (printExtendedExp (snd wm) (fst wm)) ++ "at solve (" ++ (show prunetype) ++ ")"
                   return (prunetype, [])
           else if g <= d
                    then do if matches s wm prevwm
                                then do -- putStrLn $ "I prune " ++ (printExtendedExp (snd wm) (fst wm)) ++ "at solve (match)"
                                        return (Passed, [])
                                else do let newc = Ctx (s, msegs, psystem, memlims, (wm, smap):prevwm, sargs, pm, model, coffs)
                                        -- putStrLn "going to ruleappls"
                                        (rt, proofs) <- ruleApplications newc wm g d
                                        -- putStrLn "I got the following proofs"
                                        -- mapM putStrLn $ map printProof proofs
                                        let smallestproofs = smallest costs proofs sargs
                                        let valsols = validSolutions smallestproofs
                                        return $! if null valsols
                                                      then if null smallestproofs
                                                               then (rt, [])
                                                               else (rt, [head smallestproofs])
                                                      else (rt, valsols)
                    else return (Passed, [CutOff ((expSize wm) * coffs)])
...
iterativeSolve c@(Ctx (_, _, _, _, _, sargs@(le, p, u), (_, costs), _, _)) wm d depth =
    do -- putStrLn "*******************"
       appendFile "temp.txt" ("\n\nat depth " ++ show depth ++ "\n\n")
       (rt, solutions) <- solve c wm 0 d
       -- putStrLn $ "rt is " ++ show rt
       -- putStrLn $ "nr possible solutions: " ++ (show $ length solutions)
       -- printSolutions costs solutions sargs
...
parsePM pm ps =
    let noCR = filter (\a -> a /= "" && (take 2 a) /= "//") (lines pm)
        ...
        allrules = [Substitution, FormInspect, TInspect, FInspect, MInspect, Rewrite, TRecall, CRecall, Strengthen, DeltaInspect,
        ...
        substituteRules = -- usedrules \\ [Rewrite, TRecall, CRecall, Strengthen, Weaken]
                [FormInspect, DeltaInspect, AReduceP, EReduceP]
        funcs' = concat [case rule of
                             Substitution  -> [substutiteA, substituteDelta, substituteSigma, substitutePi, forallReducePlus, existsReducePlus] -- , sigmaAndComprehension, piArrowComprehension]
        ...
        funcs = funcs' ++ (if null usedrewrites then [] else [rewrites usedrewrites]) ++ 
                          ...
                          -- ++ (if null substituteRules then [] else [substitution substituteRules])
        --funcs = funcs' ++ (if null usedrewrites then [] else [substitution usedrewrites]) ++ 
        --                  (if null usedrecalls then [] else [recall usedrecalls])
    in (funcs, wlst) 
...
main = do args    <- getArgs
          ...
          if length args >= 8  -- if there are atleast 8 arguments
              then do let mandatargs = take 8 args   -- then separate into mandatory and switch args
                      ...
                      let (l, p, u) = sargv
                      putStrLn $ "\nQuestion: " ++ (if p == TERMINAL_ then (printExp e ++ "\n") else printExp e ++ "\n")
