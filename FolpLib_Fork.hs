...
exprewrite e ps = 
    ...
    -- let mappings = (parMap rpar ) (\(EEquiv a b) -> (getPatternMapping e a [], b)) [e | e@(EEquiv a b) <- ps]
...
piInspect' c@(Ctx ((_, smp, _), _, ps, (sml, wml), _, sargs, (_, costs), model, _)) e g d =
       let sl = [(ts, (EForAll (Var v) (OptDom (tail dom)) x))
           ...
           substedlst' = (parMap rseq) (\sexp -> solve c (insertAssignMaps sexp) (pathCost costs (ENull,ENull,e) g PiInspectP sargs) d) substedlst
       in foldl fappl (Passed, []) substedlst'
...
deltaInspect c@(Ctx ((_, smp, _), _, ps, (sml, wml), _, sargs, (_, costs), model, _)) e g d =
       let sl = [(ts, case eval model (expandedExp smp tse) of Just tv -> [tv]; _ -> []) | 
           ...
           substedlst' = (parMap rseq) (\sexp -> solve c (insertAssignMaps sexp) (pathCost costs (ENull, ENull, e) g DeltaInspect sargs) d) substedlst
       in foldl fappl (Passed, []) substedlst'
                         ...
                         in if null domp
                             then []
                                  ...
                                  | ts@(EExists (Var v) (OptDom dom') (EAnd x (EPred pred)), idx) <- subformulas e False, 
                                                                                                          isColorPred pred && (not $ null dom')] 
           substedlst = [(subst se [ts] e) | (ts, se) <- sl1 ++ sl2]
           fappl = (\ret sexp ->
                        let (rt, proofs) = ret
                        in if not (rt == OPrune)
                               then    let goal = insertAssignMaps sexp
...
piArrowComprehension c@(Ctx (_, _, ps, _, _, sargs, (_, costs), model, _)) e g d =
       let sl = concat [let domp = getDom dom' model pred ETrue
                        in if null domp
                            then []
                            else [(ts, EForAll (Var v) (OptDom domp) x)] 
...
ruleApplications2 c@(Ctx (_, _, _, _, _, _, (funcs, _), _, _)) wm g d =
       let funcs' = (parMap rpar) (\f -> f c wm g d) funcs
...
-- iterativeSolve c@(Ctx (_, _, _, _, _, sargs@(le, p, u), (_, costs), _, _)) wm d depth =
    -- do cpuCores <- Control.Concurrent.getNumCapabilities 
       ...
       -- return result 
forkSolve' :: MVar Int -> MVar (Int,[TProof]) -> Ctx -> WM -> TProofSize -> IO ()
forkSolve' plist tproof c@(Ctx (_, _, _, _, _, sargs@(le, p, u), (_, costs), _, _)) wm d =
    do
        putStrLn $ "lim is " ++ show d
        ...
        if (null solutions)
           then do let unit = case u of
                   ...
                   error ""
                    ...
                    then do list <- takeMVar plist
                            putMVar plist (p + 1)
                    else do (d',proofs) <- takeMVar tproof
                            if d' == 0 || d < d'
                                then do putStrLn $ "Proof found at length " ++ show d
                                        putMVar tproof (d, nub (validSolutions solutions))
                                else putMVar tproof (d',proofs)
                            p <- takeMVar plist
                            if (p < max) then putMVar plist (p + 1) else putMVar plist p

getFree :: [(Int,Int)] -> (Int,Int)
getFree [] = error "empty thread list"
getFree xs = head . filter (\(a,b) -> b==0) $ xs

isFree :: [(Int,Int)] -> Bool
isFree xs = not . null . filter (\(a,b) -> b==0) $ xs

updateList :: [(Int,Int)] -> (Int,Int) -> [(Int,Int)]
updateList xs (a,b) = (a,b) : (filter (\(p,q) -> p /= a) xs)

forkLoop :: MVar Int -> MVar (Int,[TProof]) -> Ctx -> WM -> TProofSize -> IO ([TProof])
forkLoop plist tproof c wm d =
    do
        list <- takeMVar plist
        (len,proofs) <- readMVar tproof
        if len <= 0
          then if isFree list
                 then do let (a,_) = getFree list
                         let list' = updateList (a,d)
                         putMVar plist list'
                         forkIO $ forkSolve' plist tproof max c wm d
                         forkLoop plist tproof max c wm (d+1)
                 else do putMVar plist list
                         forkLoop plist tproof max c wm d
          else if null proofs
                then if isFree plist
                        then do
                                putMVar plist (count - 1)
                                forkIO $ forkSolve' plist tproof max c wm d
                                forkLoop plist tproof max c wm (d+1)
                        else do putMVar plist count
                                forkLoop plist tproof max c wm d
                else return proofs

-- run iterative processes on different processors in parallel
forkSolve :: Ctx -> WM -> TProofSize -> TProofSize -> IO [TProof]
forkSolve c@(Ctx (_, _, _, _, _, sargs@(le, p, u), (_, costs), _, _)) wm d depth =
    do processors <- getNumCapabilities
       tid <- myThreadId
       (p, b) <- threadCapability tid
       putStrLn $ "Root processor: " ++ show p ++ ", locked: " ++ show b
       putStrLn $ "Using " ++ show processors ++ " processors"
       plist <- newMVar (zip ([0..processors-1] \\ [p]) (repeat 0) )  -- ((processors - 1)::Int)
       let tp = ([]::[TProof])
       tproof <- newMVar (0,tp)
       -- mapM (forkIO . forkSolve' pcount tproof (processors -1) c wm) [d..(d+processors-2)]
       proofs <- forkLoop plist tproof c wm d
       return $! proofs
